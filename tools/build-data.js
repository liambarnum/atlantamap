...
function main() {
  ...
  // places.json is hand-maintained rather than generated; it only gets the .js
  // twin so the page can read it from file:// like everything else.
  const placesPath = path.join(DATA_DIR, 'places.json');
  const placesRaw = fs.readFileSync(placesPath, 'utf8');
  const placeData = JSON.parse(placesRaw);
  fs.writeFileSync(
    path.join(DATA_DIR, 'places.js'),
    `/* Generated by tools/build-data.js from places.json - do not edit. */\n` +
      `window.BELTLINE_PLACES = ${JSON.stringify(placeData, null, 2)};\n`
  );
  ...
  console.log(`  ${'places'.padEnd(26)} ${String(placeData.places.length).padStart(6)}`);
